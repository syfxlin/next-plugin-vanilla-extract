...
import { VanillaExtractWebpackPlugin } from "./vanilla-extract-webpack-plugin";
...
type PluginOptions = ConstructorParameters<typeof VanillaExtractWebpackPlugin>[0];
...
const getVanillaExtractCssLoaders = (options: WebpackConfigContext, assetPrefix: string) => {
  ...
  loaders.push({
    ...
    options: {
      ...
      url: (url: string, resourcePath: string) =>
        cssFileResolve(url, resourcePath, options.config.experimental?.urlImports),
  ...
  loaders.push({
    loader: require.resolve("./vanilla-extract-virtual-loader"),
  });

  return loaders;
};

export const createVanillaExtractPlugin = (pluginOptions: PluginOptions = {}) => {
    ...
    webpack(config: any, options: WebpackConfigContext) {
      ...
      const cssRules = config.module.rules.find(
        (rule: any) =>
          ...
          rule.oneOf.some(
            ({ test }: any) => typeof test === "object" && typeof test.test === "function" && test.test("filename.css"),
          ),
      ).oneOf;
      ...
      config.plugins.push(new VanillaExtractWebpackPlugin({ outputCss, ...pluginOptions }));
